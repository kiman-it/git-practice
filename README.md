# Git Practice 🚀

👋 Chào mình là **kiman-it** — mình đang học Git & GitHub để dùng trong team IT.

## 📌 Mục tiêu của repo này
Repository này dùng để:
- Học về git IT
- Tập commit, tạo branch
- Tập push/pull, resolve conflict
- Ghi bài tập theo từng ngày

## 📅 Nội dung sẽ làm
| Bài tập | Mô tả |
|---------|-------|
| Day 1: Git cơ bản | init, status, add, commit |
| Day 2: Push lên GitHub | git push, git clone |
| Day 3: Branch & Merge | git branch, git merge |
| … | … |


## 📌 Cách luyện
1. Sửa file trong repo
2. `git add .`
3. `git commit -m "Day X: <nội dung đã làm>"`
4. `git push`
   
## Learning Plan
- Day 1: Git basic commands
- Day 2: Push & Pull

## Tracking day one

- Yesterday I learned about basic GIT.
- 2026/1/16 Try to create new branch on main
- 2026/1/17 Recheck git edit add commit push
- 2026/1/22 Mình đã học những cái cơ bản trên github và các câu lệnh git ròi, hiện mình đang học để viết web. Minhf đang đến tầng 2, học về CSS

---

